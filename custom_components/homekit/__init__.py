...
class HomeKit:
    ...
    def _async_configure_linked_sensors(
        ...
        if state.entity_id.startswith(f"{FAN_DOMAIN}."):
            ...
            current_pm25_entity_id = device_lookup[ent_reg_ent.device_id].get(
                (SENSOR_DOMAIN, SensorDeviceClass.PM25)
            )
            if current_pm25_entity_id:
                self._config.setdefault(state.entity_id, {}).setdefault(
                    CONF_LINKED_PM25_SENSOR,
                    current_pm25_entity_id,
                )
            current_pm10_entity_id = device_lookup[ent_reg_ent.device_id].get(
                (SENSOR_DOMAIN, SensorDeviceClass.PM10)
            )
            if current_pm10_entity_id:
                self._config.setdefault(state.entity_id, {}).setdefault(
                    CONF_LINKED_PM10_SENSOR,
                    current_pm10_entity_id,
                )
